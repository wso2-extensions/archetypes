...
public class ${siddi_aggregator_name}AggregateFunctionIntegrationTest {
    private static Logger logger = Logger.getLogger(${siddi_aggregator_name}AggregateFunctionIntegrationTest.class);
    ...
    public void testProcess() throws Exception {
        logger.info("${siddi_aggregator_name}AggregateFunction TestCase");

        siddhiManager = new SiddhiManager();
        String inValueStream = "@config(async = 'true')define stream pizzaOrder (orderNo int); ";

        String eventFuseExecutionPlan = ("@info(name = 'query1') from pizzaOrder#window.time(5 sec) "
                + " select ${siddi_aggregator_name}Resource:${siddi_aggregator_name}Aggregator(orderNo) as totalOrders "
